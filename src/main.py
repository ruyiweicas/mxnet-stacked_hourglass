...
from src.utils.utils import AverageMeter, Flip, ShuffleLR
from src.utils.logger import Logger
from src.utils.eval import Accuracy, getPreds, finalPreds
...
def main():
    ...
    if opt.loadModel != 'none':
        ...
        model = HourglassNet(nModules=2,nFeats=256,nStack=2,out_num=16)
    ...
    for epoch in range(1, opt.nEpochs + 1):
        for i, (input, target, meta) in enumerate(train_loader):
            ...
            with mx.autograd.record():
                output = model(input)
                target = mx.nd.array(target,dtype='float32')
            ...
            Bar.suffix = '{split} Epoch: [{0}][{1}/{2}]| Total: {total:} | ETA: {eta:} | Loss {loss.avg[0]:.6f} | Acc {Acc.avg:.6f} ({Acc.val:.6f})'.format(epoch, i, nIters, total=bar.elapsed_td, eta=bar.eta_td, loss=Loss, Acc=Acc, split='train')
            bar.next()
        bar.finish()
